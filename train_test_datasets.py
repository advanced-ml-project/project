...
def make_data(raw_data_path, destination_folder, test_perc):
    ...
    df_train = pd.concat([df_low_train, df_med_train, df_high_train], ignore_index=True, sort=False)
    df_valid = pd.concat([df_low_valid, df_med_valid, df_high_valid], ignore_index=True, sort=False)
    df_test = pd.concat([df_low_test, df_med_test, df_high_test], ignore_index=True, sort=False)
